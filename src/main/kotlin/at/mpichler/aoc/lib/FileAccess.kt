...
internal object FileAccess {
    ...
    fun getExampleInput(year: Int, day: Int): String? {
        return getStringFromFile("$CONFIG_DIR/${getFilePrefix(year, day)}_${"%02d".format(day)}_example_input.txt")
    }

    fun saveExampleInput(year: Int, day: Int, example: String) {
        saveStringToFile("$CONFIG_DIR/${getFilePrefix(year, day)}_${"%02d".format(day)}_example_input.txt", example)
    }

    fun getInput(year: Int, day: Int): String? {
        return getStringFromFile("$CONFIG_DIR/${getFilePrefix(year, day)}_${"%02d".format(day)}_input.txt")
    }

    fun saveInput(year: Int, day: Int, input: String) {
        saveStringToFile("$CONFIG_DIR/${getFilePrefix(year, day)}_${"%02d".format(day)}_input.txt", input)
    }

    fun getAnswer(year: Int, day: Int, part: Part): String? {
        val partName = if (part == Part.A) "a" else "b"
        return getStringFromFile("$CONFIG_DIR/${getFilePrefix(year, day)}_${"%02d".format(day)}${partName}_answer.txt")
    }

    fun saveAnswer(year: Int, day: Int, part: Part, answer: String) {
        val partName = if (part == Part.A) "a" else "b"
        saveStringToFile("$CONFIG_DIR/${getFilePrefix(year, day)}_${"%02d".format(day)}${partName}_answer.txt", answer)
    }

    fun getBadAnswers(year: Int, day: Int, part: Part) : List<String> {
        val partName = if (part == Part.A) "a" else "b"
        val inputFile = File("$CONFIG_DIR/${getFilePrefix(year, day)}_${"%02d".format(day)}${partName}_bad_answers.txt")
        if (!inputFile.exists()) {
            return listOf()
        }
